...
impl Connection {
    ...
    pub fn read_u8(&mut self) -> Result<u8, SimpleDbError> {
        ...
        self.tcp_stream.read_exact(&mut buf)
            .map_err(|_| CannotDecodeNetworkMessage(String::from("Cannot read message U8")))?;
        Ok(buf[0])
    }
    ...
    pub fn read_u16(&mut self) -> Result<u16, SimpleDbError> {
        ...
        self.tcp_stream.read_exact(&mut buff)
            .map_err(|_| CannotDecodeNetworkMessage(String::from("Cannot read message U16")))?;
    ...
    pub fn read_u32(&mut self) -> Result<u32, SimpleDbError> {
        let mut buff = [0u8; 4];
        self.tcp_stream.read_exact(&mut buff)
            .map_err(|_| CannotDecodeNetworkMessage(String::from("Cannot read message U32")))?;
    ...
    pub fn read_u64(&mut self) -> Result<u64, SimpleDbError> {
        let mut buff = [0u8; 8];
        self.tcp_stream.read_exact(&mut buff)
            .map_err(|_| CannotDecodeNetworkMessage(String::from("Cannot read message u64")))?;
